...
type (
	relationModel interface {
		...
		FindOneByUserToUser(ctx context.Context, userId int64, toUserId int64) (*Relation, error)
		FindByUser(ctx context.Context, userId int64) ([]*Relation, error)
		FindByToUser(ctx context.Context, toUserId int64) ([]*Relation, error)
		Update(ctx context.Context, data *Relation) error
		Delete(ctx context.Context, id int64) error
		DeleteByUser(ctx context.Context, userId int64, toUserId int64) error
	}
...
func (m *defaultRelationModel) DeleteByUser(ctx context.Context, userId int64, toUserId int64) error {
	query := fmt.Sprintf("delete from %s where `user_id` = ? and `to_user_id` = ?", m.table)
	_, err := m.conn.ExecCtx(ctx, query, userId, toUserId)
	return err
}
...
func (m *defaultRelationModel) FindOneByUserToUser(ctx context.Context, userId int64, toUserId int64) (*Relation, error) {
...
func (m *defaultRelationModel) FindByUser(ctx context.Context, userId int64) ([]*Relation, error) {
	query := fmt.Sprintf("select %s from %s where `user_id` = ? limit 1", relationRows, m.table)
	var resp []*Relation
...
func (m *defaultRelationModel) FindByToUser(ctx context.Context, toUserId int64) ([]*Relation, error) {
	query := fmt.Sprintf("select %s from %s where `to_user_id` = ? limit 1", relationRows, m.table)
	var resp []*Relation
	err := m.conn.QueryRowsCtx(ctx, &resp, query, toUserId)
	switch err {
	case nil:
		return resp, nil
	case sqlc.ErrNotFound:
...
func (m *defaultRelationModel) Insert(ctx context.Context, data *Relation) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.UserId, data.ToUserId, data.DeleteAt)
	return ret, err
}
...
func (m *defaultRelationModel) Update(ctx context.Context, data *Relation) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.UserId, data.ToUserId, data.DeleteAt, data.Id)
	return err
}
